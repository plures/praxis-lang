...
const STEP_AND_EXPR_FORMS: &[(&str, &str)] = &[
    ...
    (
        "step_call",
        "ident (call_args | map_val | params | values)? (-> $x)?",
    ),
    ...
    (
        "step_loop",
        "loop over $x (as ident)? (key_as ident)? (-> $y)?: ... end",
    ),
    ("step_emit", "emit {..} | emit k: v ..."),
    ("step_try", "try (retry N ...)?: ... catch: ... end"),
    (
        "step_parallel",
        "parallel (-> $x)?: branch name (retry N ...)?: ... end ... end",
    ),
    ...
    (
        "code_match_stmt",
        "match code_expr { pattern => .., _ => .. }",
    ),
    ("code_try_stmt", "try { .. } catch ident? { .. }"),
    ("code_parallel_stmt", "parallel { name: { .. } ... }"),
    (
        "match_expr",
        "match subject { pattern|pattern => result, _ => default }",
    ),
];
